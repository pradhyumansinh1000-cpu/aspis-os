...
def get_student_intelligence(student_id: str):
    ...
    try:
        ...
        import json
        report_str = ai_engine.generate_student_intelligence_report(context)
        try:
...
@app.get("/api/student/{student_id}/marks")
def get_student_marks(student_id: str):
    db = next(get_student_db(student_id))
    try:
        results = db.query(models.Assessment, models.AssessmentScore).filter(
            models.Assessment.id == models.AssessmentScore.assessment_id,
            models.AssessmentScore.student_id == student_id
        ).all()
        
        marks_dict = {}
        for assessment, score in results:
            if assessment.id not in marks_dict:
                marks_dict[assessment.id] = {
                    "id": assessment.id,
            ...
            marks_dict[assessment.id]["total_score"] += score.score
            marks_dict[assessment.id]["max_score"] += score.max_score
        ...
        for mark in marks_dict.values():
            formatted_marks.append({
                ...
                "max": mark["max_score"],
                "avg": round(mark["max_score"] * 0.65, 1) # Mock class average
            })
            
        # Sort by date descending (mock)
        return {"marks": formatted_marks}
    finally:
        db.close()
